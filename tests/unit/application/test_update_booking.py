...
def make_use_case(
    ...
    user: User | None = None,
) -> tuple[UpdateBookingUseCase, AsyncMock, AsyncMock]:
    ...
    return use_case, booking_repo, user_repo
...
async def test_update_booking_success() -> None:
    ...
    use_case, booking_repo, _ = make_use_case(booking=booking, user=user)
...
async def test_update_booking_not_found() -> None:
    use_case, _, _ = make_use_case(booking=None, user=make_user())
...
async def test_update_booking_user_not_found() -> None:
    user = make_user()
    booking = make_booking(user_id=user.id)
    use_case, _, _ = make_use_case(booking=booking, user=None)
...
async def test_update_booking_unauthorized() -> None:
    ...
    use_case, _, _ = make_use_case(booking=booking, user=other_user)
...
async def test_admin_can_update_any_booking() -> None:
    ...
    booking = make_booking(user_id=owner.id)
    use_case, booking_repo, _ = make_use_case(booking=booking, user=admin)
...
async def test_update_booking_past_start_raises() -> None:
    user = make_user()
    booking = make_booking(user_id=user.id)
    use_case, _, _ = make_use_case(booking=booking, user=user)
